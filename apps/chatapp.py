...
Hybrid chat application combining client-server and peer-to-peer paradigms.
...
def get_list(headers="", body=""):
    """Get the current list of active peers from the tracker.

    Response: JSON list of peers with ip, port, username, last_seen.
    """
    with tracker_lock:
        peers = []
...
def add_list(headers="", body=""):
    """Add or update a peer in the tracker list.

    Request body: {"username": "...", "ip": "...", "port": 12345}
    Response: JSON confirmation with updated peer count.
    """
    body_data = _parse_body(body)
...
def channels(headers="", body=""):
    ...
    Public channels (members=None) are visible to everyone.
...
def create_channel(headers="", body=""):
    ...
    if members_list is not None:
        ...
        members.add(username)  # creator always included
    else:
        members = None
